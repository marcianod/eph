...
*   **Component-Based UI:** The user interface is built as a tree of reusable React components, promoting modularity and maintainability. Key components include `StatsDashboard`, `RecentMatchesTable`, `MatchRoundsTable`, `CountryStatsTable`, `ConfusionMatrix`, and `Map`.
...
*   **Unified Data Storage:** The application uses MongoDB Atlas as the primary database. It stores both the duel/game metadata and the vector embeddings for rounds. This simplifies the data architecture by removing the need for a separate Key-Value store.
